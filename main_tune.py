...
if not os.path.exists(f"{Model_name}_checkpoint_finetune/"): os.makedirs(f"{Model_name}_checkpoint_finetune/", exist_ok=True)

if os.path.exists(f"{Model_name}_checkpoint_finetune/base.pt"):
  checkpoint = torch.load(f"{Model_name}_checkpoint_finetune/base.pt")

  model.load_state_dict(checkpoint['model_state_dict'])
  
  print(f"Load Fine Tune CLIP finished!")

  print(f"input_resolution: {model.visual.input_resolution}")
  print(f"context_length: {model.context_length}")
  print(f"vocab_size: {model.vocab_size}")
  load_ep = checkpoint['epoch']
  print(f"Load Checkpoint Epoch: {load_ep}")
...
scheduler = CosineAnnealingLR(optimizer, T_max=30, eta_min=5e-9)   
...
output_file_path = f'{Model_name}_finetune_history.txt'
with open(output_file_path, 'w') as f:
    ...
    for epoch in range(EPOCH):
        ...
        for step, batch in enumerate(train_loader):
            ...
            # if(step % 10 == 0): print(f'Epoch {epoch} , Step Size: [{step} / {loader_step}], current_loss: {current_loss}')
        ...
        if epoch > 0: os.remove(f"{Model_name}_checkpoint_finetune/model_{epoch-1}.pt")
        ...
        torch.save({
            ...
            'loss': current_loss,
            }, f"{Model_name}_checkpoint_finetune/model_{epoch}.pt") #just change to your preferred folder/filename
        ...
        print(f'Epoch {epoch} , current_lr: {current_lr}, {(en-st):.2f} s, current_loss: {current_loss.item():.2f}, Accuracy: {accuracy*100:.2f}%')
        ...
        if prev_acc < current_acc:
            ...
            if best_acc < current_acc:
                if os.path.exists(f"{Model_name}_checkpoint_finetune/best.pt"): os.remove(f"{Model_name}_checkpoint_finetune/best.pt")
                ...
                'loss': current_loss,
                'acc': best_acc,
                }, f"{Model_name}_checkpoint_finetune/best.pt") #just change to your preferred folder/filename
        else:
            early_stop += 1            
    ...
    torch.save({
        ...
        'optimizer_state_dict': optimizer.state_dict(),
        'loss': current_loss,
        }, f"{Model_name}_checkpoint_finetune/final.pt") #just change to your preferred folder/filename
    ...
    plt.savefig(f'{Model_name}_checkpoint_finetune/training_loss_acc_curve.png')
...
if True:
    ...
    # print(eval_model)
    if os.path.exists(f"{Model_name}_checkpoint_finetune/{fn}"):
        checkpoint = torch.load(f"{Model_name}_checkpoint_finetune/{fn}")
        
        print(f"Load {Model_name}_checkpoint_finetune/{fn} Successed!!")
...
if True:
    ...
    plt.tight_layout()
    # Save the plotted image to a file
    plt.savefig(f'{Model_name}_checkpoint_finetune/all_class_sign_demo.png')
    ...
    # Save the plotted image to a file
    plt.savefig(f'{Model_name}_checkpoint_finetune/Cosine_similarity.png')
